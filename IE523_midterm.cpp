...
{
	// z = (X'X)^(-1)X'y
	Matrix inverseA = A.t()*A;
	inverseA = inverseA.i();
	ColumnVector z = inverseA * A.t() * y;
	return z;
	// 自己写 Write code that implements equation 1 of lesson 2 of my
...

	{
		...
		do
		{
			...
			for (int i = 0; i < A.Ncols(); i++){
				...
				if (did_i_pick_this_col[i] == true) {
					...
					for (int j = 1; j <= A.Nrows(); j++) {
						B[pick].push_back(A(j, i+1));
					}
					pick++;
				}
				// 自己写 fill this code appropriately
			}
			...
			if (Rank(Column_reduced_version_of_A) == rank_of_A)
			{
				...
				// 自己写 write code that inserts zeros in "soln" for each column
...

	{
		...
		do
		{
			...
			for (int i = 0; i < solution.Nrows(); i++) {
				if (did_i_pick_this[i] == true) {
					...
					for (int j = 1; j <= solution.Nrows(); j++) {
						B[pick].push_back(solution(j, i+1));
					}
					pick++;
				}
				// 自己写 fill this code appropriately
			...
			if (Rank(Column_reduced_version) == rank_of_solution_set)
			{
				...
				//cout << solution.Nrows() << " " << solution.Ncols() << endl;
